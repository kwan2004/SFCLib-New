...
vector<sfc_bigint>  QueryBySFC<T>::RangeQueryByRecursive_LNG_P(Rect<T> queryrect, SFCType sfc_type, int nranges, int ktimes)
{
	vector<TreeNode<T>> resultTNode;  //tree nodes correspond to queryRectangle
	TreeNode<T> root;  //root node
	root.level = 0;
...
vector<sfc_bigint>  QueryBySFC<T>::RangeQueryByBruteforce_LNG(Rect<T> queryRect, SFCType sfc_type)
{
